...
void testTreeStructure(IniSection * result) {
...
BOOST_AUTO_TEST_SUITE(RelativeNestingParsingTests)
    ...
    BOOST_AUTO_TEST_CASE(RelativeNestingWithoutChildAccessor) {
        testTreeStructure(IniParser().parse(R"(..\tests\ini\in\test2_nesting\relativeSectionNesting1.ini)").get());
    }

    BOOST_AUTO_TEST_CASE(RelativeNestingWithChildAccessor) {
        testTreeStructure(IniParser().parse(R"(..\tests\ini\in\test2_nesting\relativeSectionNesting2.ini)").get());
    }
...
BOOST_AUTO_TEST_SUITE(AbsoluteNestingParsingTests)
    ...
    BOOST_AUTO_TEST_CASE(AbsoluteNestingWithoutGlobalAccessor) {
        testTreeStructure(IniParser().parse(R"(..\tests\ini\in\test2_nesting\absoluteSectionNesting1.ini)").get());
    }
    ...
    BOOST_AUTO_TEST_CASE(AbsoluteNestingWithGlobalAccessor) {
        ...
        BOOST_TEST_MESSAGE("To run this test case, comment a matching line (currently 107) in IniParser.cpp");
//        testTreeStructure(result.get());
